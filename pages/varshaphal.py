"""Varshaphal — annual horoscope (Tajika solar-return chart)."""

from __future__ import annotations

from datetime import date, datetime, time

import streamlit as st

from astro import geo
from astro import persistence
from astro.chart_engine import BirthData, compute_chart
from astro.varshaphal import varshaphal, varshaphal_markdown

st.set_page_config(page_title="Varshaphal", page_icon="\U0001f382", layout="wide")

st.markdown(
    """
    <style>
      .stApp { background: linear-gradient(180deg,#070912,#0c1020); color:#e8ebf2; }
      h1,h2,h3 { color:#f5c542 !important; font-family:'Georgia',serif; }
      .pcard { background:rgba(28,33,54,0.55); border:1px solid rgba(245,197,66,0.14);
               border-radius:16px; padding:14px 18px; margin-bottom:10px; }
    </style>
    """,
    unsafe_allow_html=True,
)

st.markdown("# \U0001f382 Varshaphal — Annual Horoscope")
st.caption(
    "Tajika solar-return chart · Muntha · year lord (Varshesh) · month-by-month outlook "
    "· Lahiri sidereal"
)


st.markdown("### Whose chart?")
saved = persistence.list_charts()
options = ["Enter birth details"] + [f"{c['name']} — {c['summary']}" for c in saved]
choice = st.selectbox("Chart", options, key="vp_choice")

birth = None
if choice == "Enter birth details":
    with st.expander("Birth details", expanded=True):
        name = st.text_input("Name", key="vp_name", placeholder="Your name")
        c1, c2 = st.columns(2)
        with c1:
            b_date = st.date_input("Date of birth", value=date(1990, 1, 1),
                                   min_value=date(1800, 1, 1), max_value=date(2100, 12, 31),
                                   key="vp_bdate")
        with c2:
            b_time = st.time_input("Time of birth", value=time(12, 0), step=60, key="vp_btime")
        b_mode = st.radio("Birth location", ["Pick a city", "Manual lat/long"],
                          horizontal=True, key="vp_bplace_mode")
        if b_mode == "Pick a city":
            idx = geo.PLACE_NAMES.index("Rishikesh, India") if "Rishikesh, India" in geo.PLACE_NAMES else 0
            bcity = st.selectbox("Birth place", geo.PLACE_NAMES, index=idx, key="vp_bcity")
            binfo = geo.resolve_place(bcity)
            blat, blon, bplace, btz_name, btz_manual = (
                binfo.latitude, binfo.longitude, binfo.name, binfo.timezone, None)
        else:
            blat = st.number_input("Birth latitude", value=30.0869, format="%.4f", key="vp_blat")
            blon = st.number_input("Birth longitude", value=78.2676, format="%.4f", key="vp_blon")
            btz_manual = st.number_input("Birth UTC offset (hours)", value=5.5, step=0.25,
                                         format="%.2f", key="vp_btz")
            bplace, btz_name = f"{blat:.3f},{blon:.3f}", None
        if btz_name:
            btz = geo.tz_offset_hours(btz_name, datetime.combine(b_date, b_time))
        else:
            btz = btz_manual
        birth = BirthData(
            name=name, year=b_date.year, month=b_date.month, day=b_date.day,
            hour=b_time.hour, minute=b_time.minute, latitude=blat, longitude=blon,
            tz_offset=btz, place=bplace,
        )
else:
    sel = saved[options.index(choice) - 1]
    birth = persistence.load_birth(sel["id"])

year = st.number_input("Varshaphal year (solar year beginning on your birthday)",
                       min_value=1900, max_value=2100, value=date.today().year)

if st.button("Compute Varshaphal", type="primary", use_container_width=True):
    st.session_state["vp_birth"] = birth
    st.session_state["vp_year"] = int(year)

if "vp_birth" not in st.session_state:
    st.info("Pick a chart and a year, then click **Compute Varshaphal**.")
    st.stop()

chart = compute_chart(st.session_state["vp_birth"])
vp = varshaphal(chart, st.session_state["vp_year"])
native = chart.birth.name or "Native"

st.markdown(f"## {native} — Varshaphal {vp['year']} (age {vp['age']})")
st.markdown(
    f"<div class='pcard'>"
    f"<b style='color:#ffe9a8'>Solar return:</b> {vp['solar_return']} · {vp['place']}<br>"
    f"<b style='color:#ffe9a8'>Annual Ascendant:</b> {vp['annual_lagna']} "
    f"(lord {vp['annual_lagna_lord']})<br>"
    f"<b style='color:#ffe9a8'>Muntha:</b> {vp['muntha_sign']} in house {vp['muntha_house']} "
    f"(lord {vp['muntha_lord']})<br>"
    f"<b style='color:#ffe9a8'>Year lord (Varshesh):</b> {vp['year_lord']} "
    f"— {vp['year_lord_role']} ({vp['year_lord_dignity']})"
    f"</div>",
    unsafe_allow_html=True,
)

st.markdown("### Highlights")
for h in vp["highlights"]:
    st.markdown(f"- {h}")

st.markdown("### Annual chart positions")
rows = []
for p in vp["annual_planets"]:
    rows.append({
        "Planet": p["planet"] + (" \u211e" if p["retrograde"] else ""),
        "Sign": p["sign"],
        "House": p["house"],
        "Dignity": p["dignity"],
    })
st.dataframe(rows, hide_index=True, use_container_width=True)

st.markdown("### Month-by-month outlook (Sun transit)")
for m in vp["monthly"]:
    st.markdown(
        f"<div class='pcard'><b style='color:#ffe9a8'>From {m['from_date']}</b> · "
        f"Sun in {m['sun_sign']} (house {m['house']})<br>{m['theme'].capitalize()}</div>",
        unsafe_allow_html=True,
    )

st.download_button(
    "Download Varshaphal (Markdown)",
    varshaphal_markdown(vp, native),
    file_name=f"varshaphal_{vp['year']}_{native.replace(' ', '_')}.md",
    mime="text/markdown",
    use_container_width=True,
)

st.caption(
    "Tajika annual system · the year-lord uses a simplified strength proxy. "
    "For reflection and guidance."
)
