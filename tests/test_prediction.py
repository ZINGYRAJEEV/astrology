...
def test_predict_from_birth():
    ...
    assert "2. Clear picture by life area" in md
    assert "3. Your nature & behaviour" in md
    assert "4. Your story in plain words" in md
    assert "5. Life areas" in md
    assert "6. Right now" in md
    assert "7. Deeper chart signals" in md
    assert "8. Favourable elements" in md
    assert "सूर्य" in md or "Sun (सूर्य)" in md
    assert "nature_profile" in pred and len(pred["nature_profile"]["placements"]) == 9
    assert "position_brief" in pred and pred["position_brief"]["areas"]
